...
estim_extreme_quantile <- function(x, p) {
  if (length(x) < 10) stop("length(x) < 10")
  ord <- order(x, decreasing = TRUE)
  x10 <- x[ord[10]]
  if (quantile(x, 0.75, type = 1) > x10) return(NA_real_)
  results <- tibble(
...
estim_quantile <- function(x, p) {
  if (p <= 1 - 1 / length(x)) {
    return(quantile(x, p, type = 1))
  } else {
    return(estim_extreme_quantile(x, p))
  }
}